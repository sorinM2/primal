...
namespace primal::graphics::d3d12::gpass
{
	...
	bool create_gpass_pso_and_root_signature()
	{
		...
		parameters[0].as_constant(3, D3D12_SHADER_VISIBILITY_PIXEL, idx::root_constants);
		...
		struct
		{
			...
			d3dx::d3d12_pipeline_state_subobject_depth_stencil_formats depth_stencil_format{ depth_buffer_format };
		...
		NAME_D3D12_OBJECT(gpass_pso, L"GPass pso State Object");
	...
	void add_transitions_for_depth_prepass(d3dx::d3d12_resource_barier& barriers)
	{ 
	...
	void add_transitions_for_gpass(d3dx::d3d12_resource_barier& barriers)
	{
		barriers.add(gpass_depth_buffer.resource(),
			D3D12_RESOURCE_STATE_DEPTH_WRITE,
			D3D12_RESOURCE_STATE_DEPTH_READ | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
	...
	void add_transitions_for_post_process(d3dx::d3d12_resource_barier& barriers)
	{
		barriers.add(gpass_main_buffer.resource(),
			D3D12_RESOURCE_STATE_RENDER_TARGET,
			D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
