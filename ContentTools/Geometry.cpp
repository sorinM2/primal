...
namespace primal::tools
{
	...
	namespace
	{
		...
		void process_normals(mesh& m, f32 smoothing_angle)
		{
			...
			assert(num_indices and num_vertices);
		...
		void process_uvs(mesh& m)
		{
			...
			assert(num_vertices and num_indices);
		...
		void pack_vertices_static(mesh& m)
		{
			...
			assert(num_vertices);
			m.packed_vertex_static.reserve(num_vertices);
		...
		void process_vertices(mesh& m, const geometry_import_settings& settings)
		{
			...
			if (settings.calculate_normals or m.normals.empty())
			{
		...
		u64 get_mesh_size(const mesh& mesh)
		{
			...
			const u64 size
			{
				su32 + mesh.name.size() + //mesh name length and room for the string
				su32 +	//lod id
				su32 +	//vertex size
		...
		u64 get_scene_size(const scene& scene)
		{
			...
			for (auto& lod : scene.lod_groups)
			{
				u64 lod_size
				{
					su32 + lod.name.size() + //Lod name and its length
		...
		void pack_mesh_data(const mesh& mesh, u8* buffer, u64& at)
		{
			constexpr u64 su32{ sizeof(u32) };
			u32 s{ 0 };
			...
			//Lod treshhold
			memcpy(&buffer[at], &mesh.lod_treshhold, sizeof(f32)); at += sizeof(f32);

			//vertex data
			s = vertex_size * num_vertices;
			memcpy(&buffer[at], mesh.packed_vertex_static.data(), s); at += s;
